// A simple, iterative HTTP/1.0 Web server that uses pre-threading to
// implement concurrency. The number of threads changes dianymically
// with the server load, doubling of halving when the connection buffer
...
pthread_t tids[MAXTHREADS];	  // threads ids of running threads
int num_threads;							// number of active threads
sbuf_t * clients;						  // syncronized buffer of connected clients
...
void init() {
	clients = sbuf_init(MAXCLIENTS);
	for (int i = 0; i < MINTHREADS; ++i)
		pthread_create(&(tids[i]), NULL, thread_f, NULL); 
...
int main(int argc, char **argv) {
	...
	while (1) {
		if (sbuf_isfull(clients))
			double_threads();
		if (sbuf_isempty(clients))
			halve_threads();
